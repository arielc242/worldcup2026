...
const TEAM_MAP = {
  ...
  'USA':'ארהב','Paraguay':'פרגוואי','Australia':'אוסטרליה','Turkey':'טורקיה',
  'Germany':'גרמניה','Curaçao':'קוראסאו',"Côte d'Ivoire":'חוף השנהב','Ecuador':'אקוודור',
  'Netherlands':'הולנד','Japan':'יפן','Sweden':'שוודיה','Tunisia':'תוניסיה',
  'Belgium':'בלגיה','Egypt':'מצרים','Iran':'איראן','New Zealand':'ניו זילנד',
  'Spain':'ספרד','Cabo Verde':'קאבו ורדה','Saudi Arabia':'סעודיה','Uruguay':'אורוגוואי',
  'France':'צרפת','Senegal':'סנגל','Iraq':'עיראק','Norway':'נורווגיה',
  'Argentina':'ארגנטינה','Algeria':"אלג'יריה",'Austria':'אוסטריה','Jordan':'ירדן',
  'Portugal':'פורטוגל','DR Congo':'קונגו','Uzbekistan':'אוזבקיסטן','Colombia':'קולומביה',
  'England':'אנגליה','Croatia':'קרואטיה','Ghana':'גאנה','Panama':'פנמה',
  // Also map Hebrew names back to themselves (in case API returns Hebrew)
  'מקסיקו':'מקסיקו','ברזיל':'ברזיל','ארגנטינה':'ארגנטינה','צרפת':'צרפת',
  'אנגליה':'אנגליה','גרמניה':'גרמניה','ספרד':'ספרד','פורטוגל':'פורטוגל',
};

const mapTeam = n => TEAM_MAP[n] || n;

// Match number to ID mapping
...
function parseResults(data) {
  ...
  if (!data || !Array.isArray(data)) return { groupScores, koWinners };

  data.forEach(match => {
    // Only process finished matches
    if (match.status !== 'finished' && match.status !== 'completed' && match.status !== 'FT') return;

    const h = match.home_score ?? match.homeScore ?? match.score?.home ?? null;
    const a = match.away_score ?? match.awayScore ?? match.score?.away ?? null;
    if (h === null || a === null) return;

    const hNum = parseInt(h), aNum = parseInt(a);
    const matchNum = match.match_number ?? match.matchNumber ?? match.number ?? null;
    const stage = (match.stage || match.round || match.phase || '').toLowerCase();

    // Group stage
    if (matchNum && matchNum <= 72) {
      const matchId = MATCH_NUM_TO_ID[matchNum];
      if (matchId) groupScores[matchId] = { h: hNum, a: aNum };
    }

    // Knockout
    const winner = hNum > aNum ? mapTeam(match.home_team || match.homeTeam || match.team1) 
                                : mapTeam(match.away_team || match.awayTeam || match.team2);
    if (stage.includes('round of 32') || stage.includes('r32')) koWinners.r32.push(winner);
    else if (stage.includes('round of 16') || stage.includes('r16')) koWinners.r16.push(winner);
    else if (stage.includes('quarter')) koWinners.qf.push(winner);
    else if (stage.includes('semi')) koWinners.sf.push(winner);
    else if (stage.includes('third') || stage.includes('3rd')) koWinners.third = winner;
    else if (stage.includes('final') && !stage.includes('semi') && !stage.includes('third') && !stage.includes('3rd')) {
      koWinners.champion = winner;
    }
  });
